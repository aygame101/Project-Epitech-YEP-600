        ...
        (() => {
            ...
            function buildTableContainer(scene, cellW = 56, cellH = 44, gap = 6) {
                ...
                const addCell = (x, y, w, h, fill, text, meta) => {
                    ...
                    rect.on('pointerdown', () => {
                        // centre monde via bounds (correct même après rotation/scale du container)
            ...
            function placeTableRight(scene, tableContainer, labels, rightCenterX, maxRightWidth, topMargin = 16, bottomMargin = 16, yOffset = 0) {
                ...
                // marges “safe area” iOS pour ne rien couper
                const SAFE_TOP = Math.max(topMargin, 40);
                const SAFE_BOTTOM = Math.max(bottomMargin, 40);

                // bounds avant scale
                const b = tableContainer.getBounds();
                const availH = scene.scale.height - SAFE_TOP - SAFE_BOTTOM;
                const availW = Math.max(0, maxRightWidth - 8);

                const scale = Math.min(availW / b.width, availH / b.height); // léger padding
            ...
            function buildTable(scene, left, top, cellW, cellH, gap) {
                const cont = scene.add.container(left, top); // parent
                const cells = [];

                function addCell(relX, relY, w, h, fill, text, meta) {
                    const rect = scene.add
                        .rectangle(relX + w / 2, relY + h / 2, w, h, fill, 1)
                        .setStrokeStyle(3, 0xFFD700)
                        .setInteractive({ useHandCursor: true });
                    ...
                    cells.push({ key: aggKey(meta.type, meta.value), type: meta.type, value: meta.value, rect, center: { x: rect.x, y: rect.y } });
                }

                // === Construction (en coordonnées locales du container) ===
                // mêmes constantes de contenu qu'avant
                const w = cellW, h = cellH, g = gap;

                // ZERO (colonne à gauche, 3 hauteurs)
                addCell(0, 0, w, h * 3 + g * 2, 0x2ecc40, '0', { type: 'number', value: 0 });
                ...
                for (let row = 0; row < 3; row++) {
                    for (let col = 0; col < 12; col++) {
                        ...
                        const y = row * (h + g);
                        const fill = reds.has(n) ? 0xC0392B : 0x222222;
                        addCell(x, y, w, h, fill, String(n), { type: 'number', value: n });
                    }
                }

                // "2 to 1" à droite
                for (let i = 0; i < 3; i++) {
                    const x = w + g + 12 * (w + g) + g;
                    const y = i * (h + g);
                    addCell(x, y, w, h, 0xFFD700, '2 to 1', { type: 'column', value: i + 1 });
                }

                // Douzaines (sous la grille)
                const dozenY = 3 * (h + g) + 8;
                const dozenW = 4 * w + 3 * g;
                const dozenStartX = w + g;
                addCell(dozenStartX, dozenY, dozenW, h, 0xFFD700, '1st 12', { type: 'dozen', value: 1 });
                addCell(dozenStartX + dozenW + g, dozenY, dozenW, h, 0xFFD700, '2nd 12', { type: 'dozen', value: 2 });
                addCell(dozenStartX + 2 * (dozenW + g), dozenY, dozenW, h, 0xFFD700, '3rd 12', { type: 'dozen', value: 3 });

                // Extérieures
                const outY = dozenY + h + 10;
                const lowW = 2 * w + g;
                const midW = 1.5 * w;
                const seq = [
                    { w: lowW, txt: '1 to 18', type: 'low' },
                ...
                let curX = dozenStartX;
                seq.forEach(s => {
                    addCell(curX, outY, s.w, h, (s.type === 'red' ? 0xC0392B : s.type === 'black' ? 0x222222 : 0xffffff), s.txt, { type: s.type });
                    curX += s.w + g;
                });

                tableCells = cells;

                // renvoie aussi la taille "base" (utile pour le scale/placement)
                const baseWidth = 14 * w + 13 * g;            // w(zero)+gap+(12w+11g)+gap+w  => 14w + 13g
                const baseHeight = (h * 3 + 2 * g) + 8 + h + 10 + h; // explication dans ta version DOM
                cont.setSize(baseWidth, baseHeight);

                return { container: cont, baseWidth, baseHeight };
            }


            function buildChipsColumn(scene, x, anchorY, spacing = 76, align = 'center', dx = 4) {
                // nettoie l'ancienne colonne
                chipsBar.forEach(o => o.destroy && o.destroy());
                chipsBar = [];

                // on centre la colonne sur anchorY
                const values = [...CHIP_VALUES]; // ordre haut→bas
                const startY = (align === 'center')
                    ? anchorY - ((values.length - 1) * spacing) / 2
                    : anchorY; // 'bottom' = comportement ancien

                values.forEach((v, i) => {
                    const y = (align === 'center') ? (startY + i * spacing) : (anchorY - i * spacing);
                    const node = drawChip(scene, x + dx, y, v); // léger décalage à droite
            ...
            function resultOverlay(scene, winningNumber, gain, onClose) {
                ...
                // Fond assombri (bloque les interactions derrière)
                ...
                const close = drawButton(
                    ...
                    'Fermer', 0x1E88E5, '#fff',
                    () => {
                        // détruit TOUT ce qui a été créé par l’overlay
                        [shade, box, title, badge, numberText, gainText].forEach(o => o.destroy());
                        close.destroy();
                        onClose && onClose();
                    },
                    baseDepth + 3 // << profondeur du bouton
                );
            ...
            function create() {
                ...
                const rightCenterX = W - 350;   // centre de la colonne droite
                const maxRightWidth = 350;      // largeur max allouée à la table tournée
                const { container: tCont, labels } = buildTableContainer(scene, 60, 100, 6);
                const TABLE_Y_OFFSET = 600; // ↑ augmente pour descendre davantage
                ...
                const chipsX = rightCenterX - maxRightWidth / 2 - LEFT_PANEL_GAP + 100; // 38 ≈ rayon + marge
                const CHIPS_Y_OFFSET = 420; // ↑ augmente pour descendre davantage
                buildChipsColumn(scene, chipsX, H / 2 + CHIPS_Y_OFFSET, 78, 'center', 4);

                // === BOUTONS (colonne verticale, bas-gauche des chips) ===
                const btnsX = chipsX - LEFT_PANEL_GAP - 150;      // 100 ≈ demi largeur bouton (200)
